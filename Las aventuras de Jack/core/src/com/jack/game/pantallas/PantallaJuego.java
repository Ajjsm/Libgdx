...
public class PantallaJuego implements Screen {
    ...
    public void handleInput(float dt){
        if (jack.estadoActual != Jack.State.MUERTO) {
            if (Gdx.input.isKeyJustPressed(Input.Keys.UP))
                //Cambiar para dar un salto solo
                jack.jump();
            if (Gdx.input.isKeyPressed(Input.Keys.RIGHT) && jack.b2body.getLinearVelocity().x <= 2)
                jack.b2body.applyLinearImpulse(new Vector2(0.1f, 0), jack.b2body.getWorldCenter(), true);
            if (Gdx.input.isKeyPressed(Input.Keys.LEFT) && jack.b2body.getLinearVelocity().x >= -2)
                jack.b2body.applyLinearImpulse(new Vector2(-0.1f, 0), jack.b2body.getWorldCenter(), true);
        }
        else
            musica.setVolume(1);
    ...
    public void update(float dt){
        ...
        if (jack.estadoActual == Jack.State.SUPERADONIVEL){
            musica.stop();
        }
        if (jack.estadoActual == Jack.State.FINALJUEGO){
            musica.stop();
        }
        if (jack.estadoActual != Jack.State.MUERTO){

            gamecam.position.x = jack.b2body.getPosition().x;
        }
    ...
    public boolean gameOver(){
        if(jack.estadoActual == Jack.State.MUERTO && jack.getStateTimer()>3){
            return true;
        }
        return false;
    }
    ...
    public boolean nivel2(){
        if(jack.estadoActual == Jack.State.SUPERADONIVEL && jack.getStateTimer()>3){
            return true;
        }
        return false;
    }
    ...
    public boolean finalJuego(){
        if(jack.estadoActual == Jack.State.FINALJUEGO && jack.getStateTimer()>3){
            return true;
        }
        return false;
    }
